...
const refMiddleware = async (ref: any, path: string, options: AxiosRequestConfig = {}) => {
  let res: any = ref
  let count = 0
...
const checkErrorMiddleWare = (data: IResponse<any>, path: string, options: AxiosRequestConfig = {}) => {
  if (data.data?.code) {
    const errorData = {
      response: data
    }
...
export class Requester {
  ...
  public get<T> (path: string, options: AxiosRequestConfig = {}): Promise<IResponse<T>> {
    return this.service.get<T>(path, options).then((res) => checkErrorMiddleWare(res, path, options))
  }
